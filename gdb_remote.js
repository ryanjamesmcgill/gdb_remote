/**
 * GDB Debugger plugin for Cloud9 modified for remote debugging.
 *
    ...
    function main(options, imports, register) {
        ...
        function getProxySource(process){
            ...
            return PROXY
                ...
                .replace(/\{BIN\}/, bin)
                .replace(/\{PROXYPORT\}/, process.runner[0].debugport + 1)
                .replace(/\{GDBPORT\}/, configure.gdb_port)
                .replace(/\{TARGETIP\}/, configure.target.ip);
        }
        ...
        function attach(s, reconnect, callback) {
            ...
            socket.on("error", function(err) {
                console.log("gdb_remote err: ", err);
                emit("error", err);
            }, plugin);
