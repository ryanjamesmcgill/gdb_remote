...
var executable = "{BIN}";
var dirname = "{PATH}";
var gdb_port = parseInt("{GDBPORT}", 10);
var proxy_port = parseInt("{PROXYPORT}", 10);
var target_ip = "{TARGETIP}";
...
function GDB() {
    ...
    this.spawn = function() {
        this.proc = spawn('gdb-multiarch', ['-q', '--interpreter=mi2', executable], {
            detached: true,
            cwd: dirname
        });
...
server.on("error", function(err) {
    if (err.errno == "EADDRINUSE") {
        //ToDo: keep gdb from restarting itself after debugger finishes.
        exec("pkill gdb-multiarch"); // kill any hanging instances of gdb, this is a hacky solution
        exec("killall gdb-multiarch");
